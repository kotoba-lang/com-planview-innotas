(ns planview_innotas.main
  "Kotodama WASM entrypoint for the Planview_innotas clean-room actor (L4) — Clojure port.
  ...
  facts (`planview_innotas.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "planview_innotas")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"planview_innotas.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "planview_innotas-compat" :tier tier :entities entities} 200])
...
(defn start! [] :planview_innotas-compat/ready)
